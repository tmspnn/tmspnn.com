...
export default class ArticleController extends Controller {
    ...
    constructor() {
        ...
        if (this.data.my_rating) {
            this.ui("article::setStarsCount", {
                starsCount: this.data.my_rating
            });
    ...
    changeAttitudeToComment = (comment) => {
        ...
        xhr({
            ...
            success: () => {
                ...
                c.advocators_count = comment.advocated
                    ? c.advocators_count - 1
                    : c.advocators_count + 1;
                c.advocated = !c.advocated;
                this.ui(
                    "comment::onAttitudeChange",
                    c.advocated,
                    c.advocators_count
                );
            },
    ...
    reportCommentAbuse = (comment) => {
        ...
        this.ui("customSpinner::show");
        postJSON({
            url: `/api/comments/${comment.id}/abuse-reports`,
            cb: () => {
                this.toast("举报成功.");
                this.ui("comment::onAbuseReport", comment.id);
            },
            fail: () => {
                this.toast("举报失败, 请稍后再试.");
            },
            final: () => {
                this.blocked = false;
                this.ui("customSpinner::hide");
    ...
    redirectToSignIn = () => {
        this.ui("pageContainer::toPage", {
            url:
                "/sign-in?from=" +
                encodeURIComponent("/articles/" + this.data.article.id)
        });
    };
