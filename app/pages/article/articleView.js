...
export default class ArticleView extends View {
    ...
    comments = _.map(
        $$(".-comment"),
        (div, i) => new Comment("article", div, initData.comments[i])
    );
    ...
    constructor() {
        ...
        this.commentBtn.on("click", () => {
            this.dispatch("clickCommentBtn", {
                ...
                referenceId: this.referenceComment
    ...
    onNewComment = (c) => {
        this.referenceComment = null;
        this.commentInput.placeholder = "请输入评论...";
        this.commentsDiv.insertBefore(
            new Comment("article", html2DOM(c.html), c)._element,
            this.commentsDiv.firstElementChild
        );
    };
