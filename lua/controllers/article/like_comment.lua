...
local function post_advocate(_, advocator_id, comment_id)
    ...
    Article:set_hot_comment(comment.article_id, comment_id,
                            comment.advocators_count)
...
local function advocate(app, uid, comment_id)
    ...
    if already_advocated then error("duplicated.advocation", 0) end

    local comment = Comment:find_by_id(comment_id)

    if not comment then error("comment.not.exists", 0) end
...
local function rev_advocate(app, uid, comment_id)
    ...
    if not already_advocated then error("forbidden", 0) end
...
local function like_comment(app)
    ...
    if user.fame <= 0 then error("fame.too.low", 0) end
