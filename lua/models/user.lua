...
function user:set_vcode(vcode, email)
    local client = redis_client:new()
    client:run("setex", string.format("email(%s):vcode", email), vcode_ttl, vcode)
end
...
function user:get_password_sequence(email)
    local client = redis_client:new()
    return client:run("get", string.format("email(%s):password_sequence", email))
end

function user:set_password_sequence(sequence, email)
    local client = redis_client:new()
    client:run("setex", string.format("email(%s):password_sequence", email), password_sequence_ttl, sequence)
end

function user:remove_password_sequence(sequence)
    local client = redis_client:new()
    return client:run("del", string.format("email(%s):password_sequence", email))
end
