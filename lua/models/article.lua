...
function article:add_comment_advocator(comment_id, uid)
    local client = redis_client:new()
    return client:run("zadd", string.format("comments(%d):advocators", comment_id), util.timestamp(), uid)
end

function article:remove_comment_advocator(comment_id, uid)
    local client = redis_client:new()
    return client:run("zrem", string.format("comments(%d):advocators", comment_id), uid)
end

function article:add_comment_opposer(comment_id, uid)
    local client = redis_client:new()
    return client:run("zadd", string.format("comments(%d):opposers", comment_id), util.timestamp(), uid)
end

function article:remove_comment_opposer(comment_id, uid)
    local client = redis_client:new()
    return client:run("zrem", string.format("comments(%d):opposers", comment_id), uid)
end
