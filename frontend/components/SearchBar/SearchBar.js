...
export default class SearchBar extends View {
  ...
  constructor(namespace) {
    ...
    _.forEach(this.resultList.children, (li, i) => {
      const itemData = _.assign(data.recommended_keywords[i], { index: i })
    ...
    switch (e.key) {
      case "ArrowUp":
        if (this.selectedResultIdx > 0) {
          ...
          this.dispatch("changeSelectedResult", { index: this.selectedResultIdx })
        }
        break
      case "ArrowDown":
        if (this.selectedResultIdx < this.resultList.children.length - 1) {
          ++this.selectedResultIdx
          this.dispatch("changeSelectedResult", { index: this.selectedResultIdx })
        }
        break
      case "Enter":
        if (this.selectedResultIdx > -1) {
          this.ui("resultItem::onClick", { index: this.selectedResultIdx })
        }
        break
      default:
